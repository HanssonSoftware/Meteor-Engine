...
#define HRESULT_STRINGIZE(code) L#code
...
bool IDirect3DDevice::Init()
{
	...
	const Vector2<uint32> defaultWindowSize = Application::Get()->GetWindowManager()->GetFirstWindow()->GetSize();
...
bool IDirect3DDevice::CompileShader(const String shaderName, Shader& data, bool pixel)
{
	...
	if (!pixel)
	{
		...
		if (FAILED(result))
		{
			...
			MR_LOG(LogD3D11, Error, "Failed to Compile Shader! %s, %s", shaderFileName.Chr(), Layer::GetSystemLayer()->GetError().Chr());
			return false;
		}
